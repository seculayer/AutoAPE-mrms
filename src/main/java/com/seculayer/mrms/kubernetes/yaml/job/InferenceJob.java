...
public class InferenceJob extends KubeJob {
    @Override
    ...
    @Override
    public V1Job make() {
        this.metaname = String.format("%s-%s-%s", jobType, ((InferenceInfo) this.info).getInfrHistNo(), this.workerIdx);
    ...
    protected Map<String, String> makeLabels() {
        ...
        labels.put("name", String.format("%s-%s", jobType, ((InferenceInfo) this.info).getInfrHistNo()));

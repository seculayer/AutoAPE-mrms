...
public class EDAJob extends KubeJob {
    @Override
    ...
    @Override
    public V1Job make() {
        this.metaname = String.format("%s-%s-%s", jobType, this.info.getKey(), this.workerIdx);
    ...
    protected Map<String, String> makeLabels() {
        ...
        labels.put("name", String.format("%s-%s", jobType, this.info.getKey()));
    ...
    protected List<V1Container> makeContainers() {
        return this.edaContainers(jobType);
    }
}
